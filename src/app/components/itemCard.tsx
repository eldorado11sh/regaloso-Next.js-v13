...
export default function ItemCard( { status, img, background, title, description, favourite }: Props ) {
  return (
    <div className="py-8 px-3.5 w-[438px] bg-white dark:bg-[#2B2B2B] rounded-3xl relative shadow-normal">
      <div className="flex items-center justify-between relative z-10">
        <div className={classNames({
          "rounded-2xl flex py-2 px-5": true,
          "bg-[#F3F3F3] dark:bg-[#221F1F]": status,
          "bg-[#0ACF83]": !status
        })}>
          {
            status 
        ...
        <div className="rounded-2xl bg-[#F3F3F3] dark:bg-[#221F1F] p-3">
          {
      ...
      <div className="flex flex-col h-[200px]">
        <div className="flex items-center justify-between">
          <p className="text-2xl font-bold dark:text-[#F3F3F3]">{title}</p>
          {/* <IoShareSocialSharp className="text-black mr-4" size={28} /> */}
          <SocialMediaButton />
        </div>
        <p className="text-lg font-light my-2.5 dark:text-[#F3F3F3]">{description}</p>
        <button className={classNames({
          "text-lg font-semibold text-white dark:text-[#221F1F] w-fit m-auto py-3 px-8 rounded-2xl mb-0": true,
          "bg-[#D6D6D6]": !status, 
          "bg-[#0ACF83]": status
        })}>Regalatelo!</button>
      </div>
