          ...
          <div className='flex flex-col gap-[60px] items-center max-[768px]:gap-[30px]'>
            ...
            <div className='flex flex-col w-full'>
              <div className='py-[30px] border-b border-black border-opacity-20'>
                <div className='flex items-center justify-between'>
                  <div className='flex items-center gap-[30px]'>
                    <div className='px-5 pb-4 rounded-full bg-[#F3F3F3]'>
                      <BsFillCreditCardFill size={25} color="#0ACF83" />
                    </div>
                    <div className='text-[24px] font-normal'>Carta di Credito</div>
                  </div>
                  <div className='w-[40px] h-[40px] rounded-full bg-[#F3F3F3]' onClick={() => setCreditStatus(!creditStatus)} />
                </div>
                {
                  creditStatus && <div className='mt-5 px-4 py-2 rounded-[8px] border border-black border-opacity-20 flex items-center gap-12 w-fit max-[475px]:flex-wrap max-[475px]:justify-between'>
                    <div className='p-2'>
                      <FaCreditCard color="rgb(117 117 117)" size={24} />
                    </div>
                    <div className='text-[16px] font-medium text-[#252422]'>Numero Carta</div>
                    <div className='text-[16px] font-medium text-[#252422]'>MM/AA</div>
                    <div className='text-[16px] font-medium text-[#252422]'>CVC</div>
                  </div>
                }
              </div>
              ...
              <div className='py-[30px]'>
                <div className='flex items-center justify-between'>
                  <div className='flex items-center gap-[30px]'>
                    <div className='px-5 py-4 rounded-full bg-[#F3F3F3]'>
                      <FaPaypal size={25} color="#0ACF83" />
                    </div>
                    <div className='text-[24px] font-normal'>Paypal</div>
                  </div>
                  <div className='w-[40px] h-[40px] rounded-full bg-[#F3F3F3]' />
                </div>
              </div>
            </div>

            <div className='flex items-center justify-between w-full max-[588px]:flex-col max-[588px]:gap-[30px] max-[588px]:items-start'>
              <Image src={PayGroup} alt="payGroup" />
              <button className={classNames({
                "rounded-[16px] px-20 py-[14px] text-[#363636] max-[588px]:w-full": true, 
                "bg-[#0ACF83] text-[#F3F3F3]": creditStatus, 
